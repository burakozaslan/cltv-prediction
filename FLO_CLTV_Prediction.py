...
df_ = pd.read_csv(r"FLOCLTVPrediction\flo_data_20k.csv")
df = df_.copy()
df.head()
# 2. Aykırı değerleri baskılamak için gerekli olan outlier_thresholds ve replace_with_thresholds fonksiyonlarını tanımlayınız.
# Not: cltv hesaplanırken frequency değerleri integer olması gerekmektedir.Bu nedenle alt ve üst limitlerini round() ile yuvarlayınız.
...
analyse_date = last_order_date + dt.timedelta(days=2)
...
cltv["T_weekly"] = ((analyse_date - df["first_order_date"]).dt.days) / 7
...
cltv.sort_values(by="exp_sales_3_month", ascending=False)[:10]
cltv.sort_values(by="exp_sales_6_month", ascending=False)[:10]
...
ggf.conditional_expected_average_profit(cltv["frequency"],
                                        cltv["monetary_cltv_avg"]).sort_values(ascending=False).head(10)
